...
<meta name="description" content="Examples of use of scan1gen for general genome scans in R/qtl2, as well as use of scan1gen and scan1snps in scan1perm.">
...
Finally, here's how to do a permutation test. We'll just use the "loco"
method.

```{r perms_regress_loco, eval=FALSE}
operm_regress_loco <- scan1perm(iron_probs, iron$pheno, Xcovar=Xcovar,
                                scan_func=scan1gen, func=reg, kinship=k_loco,
                                n_perm=1000, cores=0)
```

```{r perms_regress_loco_really, echo=FALSE}
file <- "scan1gen_cache/perms_regress_loco.rds"
if(!file.exists(file)) {
    set.seed(20260720)
<<perms_regress_loco>>
    saveRDS(operm_regress_loco, file=file)
} else {
    operm_regress_loco <- readRDS(file)
}
```

Here's the 5% significance threshold:

```{r summary_perms_regress_loco}
...
```{r perm_snps_really, echo=TRUE}
file <- "scan1gen_cache/operm_snps.rds"
if(!file.exists(file)) {
    set.seed(20250710)
<<perm_snps>>
    saveRDS(operm_snps, file)
} else {
    operm_snps <- readRDS(file)
}
```
