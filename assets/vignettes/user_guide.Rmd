...
par(mar=c(4.1, 4.1, 1.1, 2.6), las=1)
col <- c("slateblue", "violetred", "green3")
ylim <- range(c(c2blup[,1:3], c2eff[,1:3]))+c(-1,1)
...
### QTL effects at a fixed position

To plot the estimated QTL effects at a fixed position, use
`pull_genoprobpos()` to pull out the genotype probabilities at a fixed
position, use `fit1()` to fit a single-QTL model with the QTL at that
position, and `plot_ci()` to plot the confidence intervals for the QTL
effects (as estimate &plusmn; 2 SE).

We'll consider the liver QTL on chromosome 2, at position 56.8.

```{r plot_qtleff_fixedpos}
qtl_pr <- pull_genoprobpos(pr, map, chr=2, pos=56.8)
out_fit1 <- fit1(qtl_pr, iron$pheno[,"liver"], se=TRUE)
plot_ci(out_fit1$coef[1:3], out_fit1$SE[1:3], xlab="Genotype", ylab="QTL effects")
```

### Raw phenotypes vs genotypes
...
liver phenotype (at 56.8 cM) as follows:
